...
test('band width', function () {
  const reference = new dspjs.FFT(2048, 44100)
  assert.equal(spectrum.bandWidth(2048, 44100), reference.bandwidth)
})

test('center frequency', function () {
  const reference = new dspjs.FFT(2048, 44100)
  for (let i = 0; i < 100; i += 10) {
    assert.equal(spectrum.bandFrequency(i, 2048, 44100), reference.getBandFrequency(i))
  }
})
...
test('spectrum', function () {
  ...
  const reference = new dspjs.FFT(size, 44100)
  ...
  assert.deepEqual(round(polar.magnitudes.slice(0, magnitudes.length), 5), round(magnitudes, 5))
})
