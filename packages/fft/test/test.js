...
const FFT = require('dsp-legacy').FFT
const fft = require('..')
...
test('small signal against dft', function () {
  ...
  const dftForward = dft.forward(signal)
  const result = {
  ...
  const fftForward = fft.forward(signal)
  ...
  // inverse
  assert.deepEqual(round(dft.inverse(dftForward)), round(signal))
  const fftInverse = fft.inverse(dftForward)
...
test('common signal', () => {
  ...
  const fftInverse = fft.inverse(fftForward)
  const dftInverse = dft.inverse(dftForward)
